...
baby.db <- dbConnect(RSQLite::SQLite(), "data/baby.sqlite")

SummarizedExperiment::colData(baby) %>%
  data.frame() %>%
  dbWriteTable(baby.db, "colData", .)

SummarizedExperiment::rowData(baby) %>%
  data.frame() %>%
  dbWriteTable(baby.db, "rowData", .)
...
SummarizedExperiment::assay(baby) %>%
  unpivot_assay() %>%
  dbWriteTable(baby.db, "assay", .)

dbListTables(baby.db)
